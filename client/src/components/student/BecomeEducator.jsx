  ...
  return (
    <div ref={containerRef} className="relative min-h-screen flex justify-center items-center bg-gradient-to-b from-blue-50 via-indigo-50/30 to-white overflow-hidden">
      ...
      <div className="absolute top-1/3 right-1/4 hidden lg:block">
        <div className="educator-orb"></div>
      </div>
      
      {/* Main Content */}
      <motion.div 
        className="relative p-8 z-10 flex flex-col items-start max-w-lg"
        variants={containerVariants}
        initial="hidden"
        animate="visible"
      >
        <motion.div 
          className="perspective-container mb-6"
        ...
        <motion.div 
          ...
          variants={itemVariants}
        >
          <div className="absolute inset-0 bg-gradient-to-r from-blue-500/10 via-indigo-500/10 to-blue-500/10 rounded-xl blur-xl filter opacity-70 animate-pulse-slow"></div>
          <p className="relative text-blue-800 text-lg bg-white/80 rounded-xl px-6 py-4 shadow-lg backdrop-blur-md border border-blue-100/50 transform hover:scale-105 transition-all duration-300 glassmorphism">
            Becoming an educator allows you to share your <span className="font-semibold text-indigo-600 animate-shimmer bg-clip-text text-transparent bg-[length:200%_100%] bg-gradient-to-r from-blue-600 via-indigo-500 to-blue-600">knowledge and skills</span> with students around the world.
          </p>
        </motion.div>
        ...
        {/* Features */}
        <motion.div 
          className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-12 w-full"
          variants={containerVariants}
          initial="hidden"
          animate="visible"
        >
...
const styles = `
  ...
  @keyframes float-3d {
    ...
    25% {
      transform: translateY(-10px) translateZ(20px) rotateX(4deg) rotateY(10deg);
    }
    50% {
      transform: translateY(5px) translateZ(10px) rotateX(-2deg) rotateY(-5deg);
    }
    75% {
      transform: translateY(-5px) translateZ(15px) rotateX(3deg) rotateY(8deg);
    }
  }
