...
const PaymentPage = () => {
  ...
  const fetchCourseData = async () => {
    setIsLoading(true);
    try {
      // Fetch course data without authentication first
        ...
        if (userData) {
          try {
            ...
            const enrollmentResponse = await axios.get(
              `${backendUrl}/api/user/check-enrollment/${courseId}`,
              { headers: { Authorization: `Bearer ${token}` } }
            );
            setIsAlreadyEnrolled(enrollmentResponse.data.isEnrolled);
          } catch (enrollError) {
            console.log('Enrollment check error:', enrollError);
            // Don't show toast for enrollment check errors
          }
        }
      } else {
  ...
  // Fetch educator's other courses
  const fetchEducatorCourses = async (educatorId, excludeId) => {
    setIsEducatorCoursesLoading(true);
    try {
      const { data } = await axios.get(
        `${backendUrl}/api/course/by-educator/${educatorId}?excludeId=${excludeId}`
      );
      if (data.success) {
        setEducatorCourses(data.courses);
      } else {
        console.log('Educator courses fetch warning:', data.message);
        // Don't show toast for this error as it's not critical
      }
    } catch (error) {
      console.error('Educator courses fetch error:', error);
      // Don't show toast for this error as it's not critical
    } finally {
  ...
  // Fetch educator courses when courseData is available
  useEffect(() => {
    if (courseData?.educator?._id && courseData._id) {
      fetchEducatorCourses(courseData.educator._id, courseData._id);
    }
  ...
  const paymentMethods = useMemo(() => {
    ...
    return [
      ...(courseData.paymentMethods?.ada
        ? [{ id: 'ada', name: 'Cardano Wallet', component: <AdaPayment courseData={courseData} /> }]
        : []),
      ...(courseData.paymentMethods?.stripe
        ? [{ id: 'stripe', name: 'Stripe', component: <StripePayment courseData={courseData} /> }]
        : []),
      ...(courseData.paymentMethods?.paypal
  ...
  const paginatedCourses = educatorCourses.slice(
              ...
              {courseData ? (
                <>
                  ...
                  <CourseInformationCard
                    ...
                    lectureCount={courseData.courseContent.reduce(
                        ...
                        >
                          {method.id === 'ada' && (
                            <svg
                              ...
                              viewBox="0 0 256 256"
                              fill="none"
                              stroke={selectedMethod === method.id ? 'white' : '#0033ad'}
                              strokeWidth="8"
                            >
                              <circle cx="128" cy="128" r="120" fill="none" />
                              <circle cx="128" cy="128" r="40" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="128" cy="48" r="12" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="128" cy="208" r="12" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="48" cy="128" r="12" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="208" cy="128" r="12" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="68" cy="68" r="12" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="188" cy="188" r="12" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="68" cy="188" r="12" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="188" cy="68" r="12" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="158" cy="48" r="8" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="98" cy="48" r="8" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="48" cy="98" r="8" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="48" cy="158" r="8" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="98" cy="208" r="8" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="158" cy="208" r="8" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="208" cy="158" r="8" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                              <circle cx="208" cy="98" r="8" fill={selectedMethod === method.id ? 'white' : '#0033ad'} stroke="none" />
                            </svg>
                          )}
                          {method.id === 'stripe' && (
                            <svg
                              ...
                              className="h-5 w-5"
                              viewBox="0 0 24 24"
                              fill={selectedMethod === method.id ? 'white' : '#635BFF'}
                            >
                          ...
                          {method.id === 'paypal' && (
                            <svg
                              ...
                              className="h-5 w-5"
                              viewBox="0 0 24 24"
                              fill={selectedMethod === method.id ? 'white' : '#003087'}
                            >
                ...
                <div className="bg-white p-6 rounded-lg shadow-md mt-8 border border-gray-100">
                  ...
                  {isEducatorCoursesLoading ? (
                    ...
                    <>
                      ...
                      {educatorCourses.length === 0 && (
                        <p className="text-gray-500">No more courses from this educator.</p>
                      )}
                      {totalPages > 1 && (
