...
const StudentsEnrolled = () => {
    ...
    // Nhóm sinh viên theo khóa học
    const groupedByCourse = enrolledStudents.reduce((acc, student) => {
        if (!acc[student.courseTitle]) {
            acc[student.courseTitle] = [];
        }
        acc[student.courseTitle].push(student);
        return acc;
    }, {});
        ...
        <div className='min-h-screen flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
            <div className='max-w-4xl w-full'>
                {Object.entries(groupedByCourse).map(([courseTitle, students]) => (
                    <div key={courseTitle} className="mb-6">
                        <h2 className="text-xl font-semibold mb-3 text-blue-600">{courseTitle}</h2>
